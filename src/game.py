...
class Game():
    ...
    def getDistToNextObstacle(self):
        if len(self.last_obstacle) > 0:
            return self.last_obstacle.sprites()[0].rect.left - self.dinos[0].rect.right
        else :
            return 999
    ...
    def updateGame(self):
        ...
        if len(self.cacti) < 2:
            if len(self.cacti) == 0:
                ...
                if len(self.last_obstacle) > 0 and (width-self.last_obstacle.sprites()[len(self.last_obstacle)-1].rect.right) > 70*self.gamespeed:
                    for l in self.last_obstacle:
                        if l.rect.right < width*0.7 and random.randrange(0,50) == 10:
                            #self.last_obstacle.empty()
                            self.last_obstacle.add(Cactus(self.gamespeed, 40, 40))

        if len(self.pteras) == 0 and random.randrange(0,50) == 10:# and self.counter > 500:
            # this check is to help space out some obtacles to avoid impossible-to-dodge death traps
            if len(self.last_obstacle) > 0 and (width-self.last_obstacle.sprites()[len(self.last_obstacle)-1].rect.right) > 50*self.gamespeed:
                for l in self.last_obstacle:
        ...
        for d in self.dinos:
            if not d.isDead:
                self.curScore.update(d.score, background_color)
                break
        self.highScore.update(high_score, background_color)
		
        if len(self.last_obstacle) > 0:
            if self.last_obstacle.sprites()[0].rect.left - self.dinos[0].rect.right < 0:
